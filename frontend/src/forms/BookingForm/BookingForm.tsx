...
function BookingForm({ currentUser, paymentIntent }: Props) {
    ...
    const onSubmit = async (formData: BookingFormData) => {
        if (!stripe || !elements) {
            showToast({ message: "Stripe is not properly initialized.", type: "ERROR" });
            return;
        }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-5 rounded-lg border border-slate-300 p-3">
            ...
            <div className="grid grid-cols-2 gap-2">
                <label className="text-gray-700 text-sm font-bold flex-1">
                    ...
                    <input
                        type="text"
                        className="mt-1 border rounded w-full py-3 text-gray-700 bg-gray-100"
                        readOnly
                        disabled
                        {...register("firstName")}
                    />
            ...
            <div className="space-y-2">
                ...
                <div className="bg-blue-200 p-4 rounded-md">
                    <div className="font-semibold text-lg">Total Cost: Rs.{paymentIntent.totalCost.toFixed(2)}</div>
